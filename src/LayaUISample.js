...
function onLoaded()
{
	var a = new testViewUI()
	a.tList.renderHandler = new Handler(this,onTListRender)
	Laya.stage.addChild(a);
	a.tList.array = Array.from({length:50},(a,b)=>b);
	
}

function onTListRender(item,index){
	if(!item) return;
	var t = item.getChildByName('mText');
	t.text = item.dataSource;
}
